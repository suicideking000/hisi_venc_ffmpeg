...
HI_VOID* SAMPLE_COMM_VENC_GetVencStreamProc(HI_VOID* p)
{
    ...
    FfmpegConf fc;
    char test[]="test";
    s32Ret = HI_PDT_CreateMp4(1, test, &fc);
    /******************************************
            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    if(i==1)
                    {
                        s32Ret=HI_PDT_WriteVideo(1,&stStream,&fc);
                        if(HI_SUCCESS != s32Ret)
                         {
                            SAMPLE_PRT("test err!\n");
                            return NULL;
...
HI_S32 SAMPLE_COMM_VENC_StopGetStream(void)
{
   ...
   ret = av_bsf_alloc(filter, &h264bsfc);
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        //     out_stream->codecpar-> |= AV_CODEC_FLAG_GLOBAL_HEADER;
    }
    //Output information------------------
    ...
    while (1) {
       ...
       {
       AVPacket fpkt = pkt;
       if (av_bsf_send_packet(h264bsfc, &fpkt) < 0 || av_bsf_receive_packet(h264bsfc, &fpkt) < 0) {
