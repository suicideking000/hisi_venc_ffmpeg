...
HI_VOID* SAMPLE_COMM_VENC_GetVencStreamProc(HI_VOID* p)
{
    ...
    FfmpegConf fc;
    char test[]="test";
    s32Ret = HI_PDT_CreateAVI(1, test, &fc);
    /******************************************
            ...
            for (i = 0; i < s32ChnTotal; i++)
            {
                if (FD_ISSET(VencFd[i], &read_fds))
                {
                    ...
                    *******************************************************/
                    ...
                    if(i==1)
                    {
                        s32Ret=HI_PDT_WriteVideo_AVI(1,&stStream,&fc);
                        if(HI_SUCCESS != s32Ret)
                         {
                            SAMPLE_PRT("ffmpeg get stream failed!\n");
                            return NULL;
                        }
                    }
...
HI_S32 SAMPLE_COMM_VENC_StopGetStream(void)
{
    ...
    int ret;
    // printf("START create AVI file!\n");
    // ret=HI_MP4toAVI();
    // if(ret != 0)
    // {
    //     printf("AVI file created failed!\n");
    //     goto end;
    // }
    // else
    // printf("AVI file created successful!\n");
    
//     printf("START create YUV file!\n");
//     ret= HI_MP4toYUV420P();
//     if(ret != 0)
//     {
//         printf("YUV file created failed!\n");
//         goto end;
//     }
//     else
//     printf("YUV file created successful!\n");
