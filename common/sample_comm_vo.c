...
//add
// combo_dev_cfg_t MIPI_TX_1920X1080_30_CONFIG =
// {
//     .devno = 0,
//     .lane_id = {0, 1, 2, 3},
//     .output_mode = OUTPUT_MODE_DSI_VIDEO,
...
//     },
//     .phy_data_rate =496,
//     //.phy_data_rate =891, //2023.7.22modefy
//     .pixel_clk = 74250,
// };
//add
...
HI_S32 SAMPLE_COMM_VO_StartDev(VO_DEV VoDev, VO_PUB_ATTR_S* pstPubAttr)
{
	...
	/* Fill pub attr */
    pstPubAttr->u32BgColor = 0x00ff00;
	pstPubAttr->enIntfType = VO_INTF_MIPI; //接口类型
	pstPubAttr->enIntfSync = VO_OUTPUT_USER; //时序类型
...
************************************/
    ...
    pstPubAttr->stSyncInfo.u8Intfb = 1;  	
	pstPubAttr->stSyncInfo.u16Vact = 1080;	// 垂直有效区域，单位行
	pstPubAttr->stSyncInfo.u16Vbb = 41; 	// 垂直消隐后肩，单位行
	pstPubAttr->stSyncInfo.u16Vfb = 4; 	    // 垂直消隐前肩，单位行
	pstPubAttr->stSyncInfo.u16Hact = 1920;	// 水平有效区域，单位像素
	pstPubAttr->stSyncInfo.u16Hbb = 192; 	// 水平消隐后肩，单位像素
	pstPubAttr->stSyncInfo.u16Hfb = 88; 	// 水平消隐前肩，单位像素
	...
	pstPubAttr->stSyncInfo.u16Hpw = 44; 	// 水平同步信号宽度，单位像素
	pstPubAttr->stSyncInfo.u16Vpw = 5; 	    // 垂直同步信号宽度，单位行
...
************************************/
	/* Fill user sync info */
	stUserInfo.stUserIntfSyncAttr.enClkSource = VO_CLK_SOURCE_PLL; //时钟源选择

	stUserInfo.stUserIntfSyncAttr.stUserSyncPll.u32Fbdiv = 148;		//PLL整数倍频系数 0-0xfff
	stUserInfo.stUserIntfSyncAttr.stUserSyncPll.u32Frac= 0x800000;
	...
	s32Ret = HI_MPI_VO_Enable(VoDev);  //使能设备
	if (s32Ret != HI_SUCCESS)
	{
		SAMPLE_PRT("failed with %#x!\\n", s32Ret);
		return HI_FAILURE;
...
#else
    //pstVoConfig->enVoIntfType = VO_INTF_HDMI; //7.27
    pstVoConfig->enVoIntfType = VO_INTF_MIPI; //7.10 modefy
#endif
    pstVoConfig->enIntfSync        = VO_OUTPUT_USER; //modefy
    //pstVoConfig->enIntfSync        = VO_OUTPUT_1080P60; //7.23
    pstVoConfig->u32BgColor        = COLOR_RGB_BLUE; 
    pstVoConfig->enPixFormat       = PIXEL_FORMAT_YVU_SEMIPLANAR_420; //7.28
...
HI_S32 SAMPLE_COMM_VO_StartVO(SAMPLE_VO_CONFIG_S *pstVoConfig)
{
    ...
    ********************************************/
    ...
    //VO_INTF_TYPE_E         enVoIntfType   = VO_INTF_HDMI; //7.23
    VO_INTF_TYPE_E         enVoIntfType   = VO_INTF_MIPI; //7.10modefy
    //VO_INTF_SYNC_E         enIntfSync     = VO_OUTPUT_1080P60;  
    VO_INTF_SYNC_E         enIntfSync     = VO_OUTPUT_USER; //7.10modefy 
   ...
   //2023.7.23add
    ...
    if(HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("HI_MPI_VO_getPubAttr failed!\n");
        return s32Ret;
    }
   //
...
HI_S32 SAMPLE_COMM_VO_StartPIP(SAMPLE_VO_CONFIG_S *pstVoConfig)
{
    ...
    ********************************************/
    ...
    //VO_INTF_SYNC_E         enIntfSync     = VO_OUTPUT_720P60; //7.27
    VO_PART_MODE_E         enVoPartMode   = VO_PART_MODE_SINGLE;
    VO_PUB_ATTR_S          stVoPubAttr    = {0};
    VO_VIDEO_LAYER_ATTR_S  stLayerAttr    = {0};
    HI_S32                 s32Ret         = HI_SUCCESS;
...
HI_S32 SAMPLE_COMM_VO_GetDefLayerConfig(SAMPLE_COMM_VO_LAYER_CONFIG_S * pstVoLayerConfig)
{
    ...
    //pstVoLayerConfig->enIntfSync     = VO_OUTPUT_720P60; //7.27
    pstVoLayerConfig->enIntfSync     = VO_OUTPUT_USER; //7.10modefy
...
//8.14add
/*static HI_VOID SAMPLE_PRIVATE_VO_InitScreen1920x1080(HI_S32 fd)
{
    HI_S32 s32Ret;
    cmd_info_t cmd_info;

    cmd_info.devno = 0;
    cmd_info.cmd_size = 0xaacd;
    cmd_info.data_type = 0x23;
    cmd_info.cmd = NULL;
    s32Ret = ioctl(fd, HI_MIPI_TX_SET_CMD, &cmd_info);
    if (HI_SUCCESS != s32Ret)
...
static void SAMPLE_PRIVATE_VO_InitMipiTxScreen(VO_INTF_SYNC_E enVoIntfSync, HI_S32 fd)
{
    if( VO_OUTPUT_1080x1920_60 == enVoIntfSync ||
        VO_OUTPUT_1080P30      == enVoIntfSync)
    {
