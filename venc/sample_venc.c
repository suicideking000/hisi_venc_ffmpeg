...
HI_S32 SAMPLE_VENC_SetClkCfg(HI_U32 u32ClkCfg)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32Value;
    HI_U32 u32Mask = 0x7;

    s32Ret = SAMPLE_SYS_GetReg(g_u64VeduClkAddr,&u32Value);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Get Reg failed!\n");
        return HI_FAILURE;
    }

    u32Value &= ~u32Mask;
    u32Value |= u32ClkCfg & u32Mask;

    s32Ret = SAMPLE_SYS_SetReg(g_u64VeduClkAddr, u32Value);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Set Reg failed!\n");
        return HI_FAILURE;
    }
    return HI_SUCCESS;
}
...
void SAMPLE_VENC_Usage(char* sPrgNm)
{
    ...
    printf("index:\n");
    printf("\t  0) H.265e@4K@30fps + H264e@720p@30fps.\n");
    printf("\t  1) Lowdelay:H.265e@4K@30fps.\n");
    printf("\t  2) Qpmap:H.265e@4K@30fps + H264e@4K@30fps.\n");
    printf("\t  3) IntraRefresh:H.265e@4K@30fps(Row) + H264e@4K@30fps(Column).\n");
    printf("\t  4) RoiBgFrameRate:H.265@4K@30fps + H.264@4K@30fps.\n");
    printf("\t  5) DeBreathEffect:H.265@4K@30fps + H.264@4K@30fps.\n");
    printf("\t  6) svc-t :H.264@4K@30fps.\n");
    printf("\t  7) Mjpeg@4K.\n");
    printf("\t  8) Jpeg@4K + Thumbnail.\n");
    return;
}
...
void SAMPLE_VENC_HandleSig(HI_S32 signo)
{
    ...
    {
        SAMPLE_VENC_SetClkCfg(g_u32DefaultClkCfg);
...
/******************************************************************************
* function :  H.265e@4K@30fps + H264e@720p@30fps
******************************************************************************/
HI_S32 SAMPLE_VENC_4K30(void)
{
    HI_S32 i;
    HI_S32 s32Ret;
    SIZE_S          stSize[2];
    PIC_SIZE_E      enSize[2]     = {PIC_400x400, PIC_400x400};
    HI_S32          s32ChnNum     = 2;
...
HI_S32 SAMPLE_VENC_LOW_DELAY_4K(void)
{
    ...
    HI_S32 s32Ret;
    SIZE_S          stSize[2];
    PIC_SIZE_E      enSize[2]     = {PIC_SIZE,PIC_SIZE};
...
/******************************************************************************
* function:  H.265e@4K@30fps + H264e@4K@30fps
******************************************************************************/
HI_S32 SAMPLE_VENC_IntraRefresh_4K(void)
{
   ...
   /***encode h.265 **/
    ...
    stIntraRefresh.bRefreshEnable = HI_TRUE;
    stIntraRefresh.enIntraRefreshMode = INTRA_REFRESH_COLUMN;
    stIntraRefresh.u32RefreshNum = (stSize[1].u32Width + 15)>>6;
    stIntraRefresh.u32ReqIQp = 30;
...
/******************************************************************************
* function:  H.265@4K@30fps + H.264@4K@30fps
******************************************************************************/
HI_S32 SAMPLE_VENC_ROIBG_4K(void)
{
    HI_S32 i;
    HI_S32 s32Ret;
    SIZE_S          stSize[2];
...
* function:  H.265e@4K@30fps + H264e@4K@30fps
******************************************************************************/
HI_S32 SAMPLE_VENC_DeBreathEffect_4K(void)
{
    HI_S32 i;
    HI_S32 s32Ret;
    SIZE_S          stSize[2];
...
/******************************************************************************
* function: Qpmap:H.265e@4K@30fps + H264e@4K@30fps
******************************************************************************/
...
HI_S32 SAMPLE_VENC_MJPEG(void)
{
    ...
    VENC_CHN        VencChn[2]    = {0,1};
    HI_U32          u32Profile[2] = {0,0};
    PAYLOAD_TYPE_E  enPayLoad[2]  = {PT_MJPEG};
    VENC_GOP_MODE_E enGopMode     = VENC_GOPMODE_NORMALP;
    VENC_GOP_ATTR_S stGopAttr;
    SAMPLE_RC_E     enRcMode;
...
/******************************************************************************
* function:  Thumbnail of 1*4K jpeg
******************************************************************************/
HI_S32 SAMPLE_VENC_JPEG_Thumb(void)
{
    HI_S32 i;
    HI_U8  ch;
    HI_S32 s32Ret;
    ...
    ******************************************/
    ...
    /***encode Jpege **/
    s32Ret = SAMPLE_COMM_VENC_SnapStart(VencChn[0], &stSize[0], bSupportDcf);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Venc Start failed for %#x!\n", s32Ret);
        goto EXIT_VI_VPSS_UNBIND;
    ...
    ******************************************/
    ...
    {
        s32Ret = SAMPLE_COMM_VENC_SnapProcess(VencChn[0], 1, HI_TRUE, HI_TRUE);
        if (HI_SUCCESS != s32Ret)
        {
...
#endif
{
    ...
    /*Get the default  frequency point */
    s32Ret = SAMPLE_SYS_GetReg(g_u64VeduClkAddr,&u32Value);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Get Reg failed!\n");
    ...
    switch (u32Index)
    {
        ...
        case 3:
            ...
            break;
        case 4:
            s32Ret = SAMPLE_VENC_ROIBG_4K();
            break;
        case 5:
            s32Ret = SAMPLE_VENC_DeBreathEffect_4K();
            break;
        case 6:
            s32Ret = SAMPLE_VENC_SVC_H264();
            break;
        case 7:
            s32Ret = SAMPLE_VENC_MJPEG();
            break;
        case 8:
            s32Ret = SAMPLE_VENC_JPEG_Thumb();
            break;
        default:
    ...
    /*Restore the default  frequency point when the business exits*/
    SAMPLE_VENC_SetClkCfg(g_u32DefaultClkCfg);
